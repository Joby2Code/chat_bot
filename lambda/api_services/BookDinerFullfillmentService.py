...
dining_request_queue_url = 'https://sqs.us-east-1.amazonaws.com/661219094067/dining_request_queue'
...
def constructQueueMessage(location, date, cuisine, time, number, contact):
    number = str(number)
    contact = str(contact)
    message_attributes = {
        'Location': {
...
def fullfill_dining_request(intent_request):
    ...
    message_attributes = constructQueueMessage(location, date, cuisine, time, number, contact)
    response = send_msg_to_queue(message_attributes=message_attributes, contact=contact)

    if response['MessageId']:
        print('Fullfillment done')
        content = 'Thank you. We will look for the best suggestion given the requirement and send you the details via SMS at {}'.format(
            contact)
        return close(
            session_attributes,
            'Fulfilled',
